...
export const Reticulum = (function() {
  ...
  fuse.initiate = function(options) {
    ...
    this.clickCancel =
      parameters.clickCancelFuse === undefined
        ? false
        : parameters.clickCancelFuse; //default to false;
    this.phiSegments = 3;
  ...
  reticle.initiate = function(options) {
    ...
    var geometryScale = new THREE.RingGeometry(
      this.innerRadiusTo,
      this.outerRadiusTo,
      32,
      3,
  ...
  reticle.setDepthAndScale = function(depth) {
    ...
    crosshair.position.z =
      utilities.clampBottom(z, settings.camera.near + 0.1) * -1;
  ...
  var detectHit = function() {
    try {
      ...
      raycaster.ray.direction
        ...
        .unproject(settings.camera)
        .sub(settings.camera.position)
        .normalize();
    }
  ...
  var gazeOver = function(threeObject) {
    ...
    reticle.colorTo =
      threeObjectData.reticleHoverColor || reticle.globalColorTo;
  ...
  var gazeClick = function(threeObject) {
    var clickCancelFuse =
      threeObject.reticulumData.clickCancelFuse != null
        ? threeObject.reticulumData.clickCancelFuse
        : fuse.clickCancel;
    //Cancel Fuse
    if (clickCancelFuse) {
      //Reset the clock
  ...
  return {
    add: function(threeObject, options) {
      ...
      if (parameters.reticleHoverColor) {
        threeObject.reticulumData.reticleHoverColor = new THREE.Color(
          parameters.reticleHoverColor
        );
      }
      //Fuse
      ...
      threeObject.reticulumData.fuseVisible =
        parameters.fuseVisible === undefined ? null : parameters.fuseVisible;
      threeObject.reticulumData.clickCancelFuse =
        parameters.clickCancelFuse === undefined
          ? null
          : parameters.clickCancelFuse;
      //Events
    ...
    init: function(camera, options) {
      ...
      if (!c instanceof THREE.Camera) {
        console.error(
          "ERROR: Camera was not correctly defined. Unable to initiate Reticulum."
        );
        return;
      }
      initiate(c, o);
